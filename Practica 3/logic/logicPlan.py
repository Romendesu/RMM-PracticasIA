...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    # Si hay una pared, pacman no puede estar ahi
...
def checkLocationSatisfiability(x1_y1: Tuple[int, int], x0_y0: Tuple[int, int], action0, action1, problem):
    ...
    # Añadimos axiomas de física para timestep 0 y 1
    ...
    # Declaramos la ubicación inicial y las acciones conocidas
    KB.append(PropSymbolExpr(pacman_str, x0, y0, time=0)) # Pacman's current location (x0,y0)
    KB.append(PropSymbolExpr(action0, time=0))             # Acción tomada en t=0
    KB.append(PropSymbolExpr(action1, time=1))             # Acción tomada en t=1


    # Consultamos al SAT solver si Pacman está en (x1, y1) en t=1
    model1 = findModel(conjoin(KB) & PropSymbolExpr(pacman_str, x1, y1, time=1))

    # Consultamos al SAT solver si Pacman NO está en (x1, y1) en t=1
